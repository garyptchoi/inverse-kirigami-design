function R = R_boundary_rings_generic_3D(points, angles, rings)
    ...
    for i = 1:num_rings
        ...
        for j = 1:length(rings{i,1})
            ...
            this_angle = atan2_angle_3D(points(angles(rings{i,1}(j),1),:), points(angles(rings{i,1}(j),2),:), points(angles(rings{i,1}(j),3),:));           
            % store this angle
            R(i) = R(i) + this_angle;
