function rows = C_boundary_ring_rows_generic(points, angles, rings, fixed)
    ...
    for i = 1:num_rings
        ...
        for j = 1:length(rings{i,1})
            ...
            this_grad = angle_grad(points(angles(rings{i,1}(j),1),:), ...
                                   points(angles(rings{i,1}(j),2),:), ...
                                   points(angles(rings{i,1}(j),3),:)); 
            
            % get node indices of this angle            
            nodes = [(angles(rings{i,1}(j),1)-1)*2+1 (angles(rings{i,1}(j),2)-1)*2+1 (angles(rings{i,1}(j),3)-1)*2+1];
