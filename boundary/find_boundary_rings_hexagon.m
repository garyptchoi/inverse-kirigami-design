function boundary_rings = find_boundary_rings_hexagon(pointsD, anglesD, Dto0, free, cornersD)
% prepare initial space boundary constraint stencils for hexagon pattern
% note: it contains all angles (as the hexagon pattern has holes)
    ...
    for i = 1:length(boundary_rings)    
        boundary_rings{i,2} = 2*pi/3*length(boundary_rings{i,1});
