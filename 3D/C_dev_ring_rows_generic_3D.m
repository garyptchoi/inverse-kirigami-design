function rows = C_dev_ring_rows_generic_3D(points, angles, rings, fixed)
    ...
    for i = 1:num_rings
        ...
        for j = rings(i,1):rings(i,2)
            ...
            this_grad = angle_grad_3D(points(angles(j,1),:), points(angles(j,2),:), points(angles(j,3),:));
            
            % get node indices of this angle            
            nodes = [(angles(j,1)-1)*3+1 (angles(j,2)-1)*3+1 (angles(j,3)-1)*3+1];
