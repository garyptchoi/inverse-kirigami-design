function rows = C_edge_pair_rows(points, edges, edge_pairs, fixed)
    ...
    % index into rows
    col_indA1 = (edges(edge_pairs(:,1),1)-1)*2+1;
    col_indA2 = (edges(edge_pairs(:,1),2)-1)*2+1;
    
    col_indB1 = (edges(edge_pairs(:,2),1)-1)*2+1;
    col_indB2 = (edges(edge_pairs(:,2),2)-1)*2+1;
    ...
    iii = [indexA1,indexA2,indexB1,indexB2,indexA1,indexA2,indexB1,indexB2]';
    ...
    rows = sparse(iii,jjj,kkk,num_pairs,2*num_active_points);
    rows = full(rows);
