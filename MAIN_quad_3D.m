% Main program for designing generalized quad kirigami patterns for fitting
% 3D target surfaces
%
...
[solved_pointsD, ~, ~, ~] = fmincon(@(x)OBJ_regularization_3D( ...
    ...
    [], [], [], [], [], [], ... linear constraints
    @(x)all_constraint_residual_and_jacobian_3D( ... 
    decompose_v_3D(x), face_setsD{1},... initial point and quad
...
solved_points0 = get_contracted_shape_3D(solved_pointsD, face_setsD, Dto0);
...
name = strcat('results_3D/quad_',shape_name, '_w', num2str(width), '_h', num2str(height),...
